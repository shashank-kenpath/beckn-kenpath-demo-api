...
class DatabaseService {
  ...
  async search(params = {}) {
    ...
    try {
      ...
      if (params.query) {
        paramCount += 3;
        query += ` AND (p.name ILIKE $${paramCount-2} OR p.description ILIKE $${paramCount-1} OR p.category ILIKE $${paramCount})`;
      ...
      query += `
        ...
        SELECT 
          ...
          null as organic,
          null as images,
          s.equipment_included as specifications,
          f.id as provider_id,
          f.name as provider_name,
          f.city as provider_city,
          f.state as provider_state,
      ...
      if (params.query) {
        const searchTerm = `%${params.query}%`;
        query += ` AND (s.name ILIKE $${paramCount-2} OR s.description ILIKE $${paramCount-1} OR s.category ILIKE $${paramCount})`;
      }

      if (params.category) {
        query += ` AND s.category = $${paramCount-2}`;
      }

      if (params.location) {
        query += ` AND f.city = $${paramCount-1}`;
      }
